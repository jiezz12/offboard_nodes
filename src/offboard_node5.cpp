...
double e = 0, f = 0, h = 2, a = 0, b = 0, a1 = 0, b1 = 0;
double distance = 0;
...
target_yaw = 0;
e = local_pos.pose.position.x;
f = local_pos.pose.position.y;
while(ros::ok())//进入大循环
{
  if (current_state.mode != "OFFBOARD" && (ros::Time::now() - last_request > ros::Duration(5.0)))
	...
	else
	{
		...
		{
			case 0:
				setpoint.position.x = e;
				setpoint.position.y = f;
				setpoint.position.z = h;
			...
			*/
			case 2:
				a = a1 ;
				b = b1 + 30;
				distance = sqrt(pow(local_pos.pose.position.x - a1,2) + pow(local_pos.pose.position.y - b1,2));
				ROS_INFO("distance:%f",distance);
				setpoint.position.x = a;
				setpoint.position.y = b;
				setpoint.position.z = h;
				if (std::abs(distance - 30) < 0.5)
                  		{
     		  	                if (sametimes > 20)
              				{
						a1 = local_pos.pose.position.x;
