...
#define RADIUS          5   // 绕圆运动的半径大小 m
#define CYCLE_S         20 // 完成一次绕圆运动所花费的时间
#define STEPS           (CYCLE_S*RATE)
#define VEL_Y		0.03	//y轴速度
...
void init_step()
{
    ...
    for(i=0;i<STEPS;i++)
    {
        ...
        path0[i].position.y =  i * VEL_Y + b1;
        path0[i].position.z =  c1 - r * s;

	path1[i].position.x =  a1 ; 
        path1[i].position.y =  i * VEL_Y + b2;
	path1[i].position.z =  c1;
/*
